...
// TODO: setCapacity method
// TODO: clear/reset method

/**
 * A vertically-scrolling circular {@link Buffer}.  This buffer exposes
 * virtual row indexes.  As the content scrolls, the row coordinate of any
 * particular cell remains constant.  As more rows are added to the bottom of
 * the buffer, row indexes scroll off the top and become inaccessible.
 */
public class ScrollbackBuffer implements Buffer {
	 ...
	 * @param attributes
	 */
	synchronized public void write(int column, int row, CharSequence seq, int off, int len, int attributes) {
	 ...
	 * @param attributes
	 */
	synchronized public void write(int column, int row, CharSequence seq, int attributes) {
	 ...
	 */
	@Override
	synchronized public void removeBufferObserver(BufferObserver observer) {
	 ...
	 */
	synchronized public void advance(int row) {
		if(row < extents.y) throw new IndexOutOfBoundsException();
	...
	protected void fireContentChanged(int column, int row, int width, int height) {
	...
	protected void fireExtentsChanged(int column, int row, int width, int height) {
		int size = observers.size();
		for(int i = 0; i < size; ++i) {

...
public class SwingDisplay extends JComponent implements BufferObserver, StickyScrollable {
	...
	public SwingDisplay(Buffer buffer, SoftFont font, int columns, int rows) {
		this(buffer, font, columns, rows, true);
	}
	
	public SwingDisplay(Buffer buffer, SoftFont font, int columns, int rows, boolean blink) {
		this.buffer = buffer;
